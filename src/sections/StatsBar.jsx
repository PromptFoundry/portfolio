  ...
  return (
    <section className="border-b" style={{ borderColor: 'var(--color-border)' }}>
      <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4 divide-x divide-y md:divide-y-0" style={{ borderColor: 'var(--color-border)' }}>
        {stats.map((s, i) => (
          <div key={i} className="px-4 sm:px-8 py-4 sm:py-6 flex flex-col items-center text-center" style={{ borderColor: 'var(--color-border)' }}>
            <span className="font-serif text-2xl sm:text-3xl font-light mb-1" style={{ color: 'var(--color-foreground)' }}>{s.value}</span>
